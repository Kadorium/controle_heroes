...
export function exportProductsCsv(rows: Product[]) {
  const header = [
    "sku_code", "description", "product_group", "product_subgroup", "size", "color",
    "lifecycle_status", "ncm",
    "supplier_code", "default_supplier_name", "weight_kg", "volume_m3",
  ];
  const lines = rows.map((r) =>
    [
      r.sku_code, r.description, r.product_group ?? "", r.product_subgroup ?? "",
      r.size ?? "", r.color ?? "", r.lifecycle_status ?? "",
