...
def _build_group_parsed(
    ...
    *,
    ...
    exact = match_product(db, primary)
    exact_slug = exact if exact is not None and is_slug_shell(exact) else None
    ...
    if candidates:
        ...
        parsed["suggestion_badge"] = triage_suggestion_badge(
            confidence=float(top.score),
            suggested_product=top.product,
            exact_slug=exact_slug,
        )
    elif exact_slug is not None:
        parsed["suggested_product_id"] = exact_slug.id
        parsed["suggested_product_sku"] = exact_slug.sku_code
        parsed["suggested_product_description"] = exact_slug.description
        parsed["match_confidence"] = MATCH_SUGGEST_MIN_SCORE
        parsed["match_reason"] = "match exato em produto slug"
        parsed["suggestion_badge"] = "verify"
    return parsed


def _triage_suggested_product_id(data: dict) -> int | None:
    """Sugestão para UI: pré-seleciona combobox quando confiança >= 30."""
...
def sync_heroes_xlsx_staging(
    ...
    *,
    ...
    for group in groups.values():
        ...
        if not parsed.get("resolved_product_id"):
    ...
    preview["sku_draft_aliases"] = _collect_sku_draft_aliases(
        db, run_id=run_id, raw_file_id=raw_file_id
    )
    return preview


def _collect_sku_draft_aliases(db: Session, *, run_id: int, raw_file_id: int) -> list[str]:
    ...
    for staging in rows:
        ...
        if data.get("heroes_run_id") != run_id or data.get("source") != "heroes_xlsx":
            continue
        resolved_id = data.get("resolved_product_id")
        if not resolved_id:
            continue
        prod = db.query(Product).filter(Product.id == resolved_id).first()
        if not prod or prod.lifecycle_status != LIFECYCLE_DRAFT:
            continue
        for name in data.get("aliases") or [data.get("product_name_raw")]:
            if name:
...
def build_sku_triage_groups(db: Session, *, run_id: int, raw_file_id: int) -> list[dict]:
    """Todos os grupos da importação — confirmados e pendentes — para o painel de triage."""
    rows = (
        db.query(StagingImportRow)
        .filter(StagingImportRow.raw_file_id == raw_file_id)
        .all()
    ...
    for staging in rows:
        ...
        if staging.status == StagingRowStatus.MERGED.value:
            continue
        resolved_id = data.get("resolved_product_id")
        triage_suggested = None if resolved_id else _triage_suggested_product_id(data)
        confidence = data.get("match_confidence")
        groups.append(
            {
                ...
                "suggestion_badge": data.get("suggestion_badge") or "none",
                "resolved_product_id": resolved_id,
                "resolved_sku_code": data.get("resolved_sku_code"),
                "status": "confirmed" if resolved_id else "pending",
            }
        )
