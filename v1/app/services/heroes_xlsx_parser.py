...
def _is_da_spedire_marker(cell: Any) -> bool:
    if cell is None:
        return False
    return _norm_header(cell) == "da spedire"


def _is_da_spedire_footer_text(name: str) -> bool:
    low = name.lower().strip()
    if len(low) > 35:
        return True
    hints = ("quindi", "acconto pagato", "fattura", "racchette sulle", "spedite")
    return any(h in low for h in hints)


def _find_da_spedire_marker_col(row: list[Any]) -> int | None:
    for ci, cell in enumerate(row):
        if _is_da_spedire_marker(cell):
            return ci
    return None


def _has_listino_style_columns(row: list[Any], marker_col: int) -> bool:
    for off in range(3, 7):
        ci = marker_col + off
        if ci >= len(row):
            break
        v = parse_it_number(row[ci])
        if v is not None and Decimal("0.01") <= v <= Decimal("9999"):
            return True
    return False


def _infer_inline_da_spedire_col_map(
    grid: list[list[Any]], start: int, marker_col: int
) -> dict[str, int]:
    """Layout ordine 132: marcador 'da spedire' na coluna do produto, sem linha de cabeçalho."""
    col_map: dict[str, int] = {"product": marker_col}
    sample_row: list[Any] | None = None
    for dr in range(start + 1, min(start + 15, len(grid))):
        row = grid[dr]
        p = _cell_str(row[marker_col] if marker_col < len(row) else None)
        if p and not _is_da_spedire_marker(p) and not _is_da_spedire_footer_text(p):
            sample_row = row
            break
    if sample_row and _has_listino_style_columns(sample_row, marker_col):
        col_map["quantity"] = marker_col + 1
        col_map["acconto"] = marker_col + 2
        col_map["credit_remaining"] = marker_col + 3
        col_map["price_listino"] = marker_col + 4
        col_map["invoice_price"] = marker_col + 5
        col_map["discount"] = marker_col + 6
    else:
        if marker_col > 0:
            col_map["quantity"] = marker_col - 1
        col_map["acconto"] = marker_col + 1
        col_map["credit_remaining"] = marker_col + 2
    return col_map


def _parse_da_spedire_row(
    row: list[Any], ri: int, col_map: dict[str, int]
) -> dict[str, Any] | None:
    product = _cell_str(_get_col(row, col_map, "product"))
    if not product or _is_da_spedire_marker(product) or _is_da_spedire_footer_text(product):
        return None
...
def _parse_da_spedire(grid: list[list[Any]]) -> tuple[list[dict], list[str]]:
    ...
    if header_row is None:
        marker_col = _find_da_spedire_marker_col(grid[start])
        if marker_col is None:
            return [], ["Cabeçalho DA SPEDIRE não encontrado"]
        col_map = _infer_inline_da_spedire_col_map(grid, start, marker_col)
        header_row = start
    ...
    for ri in range(header_row + 1, len(grid)):
        row = grid[ri]
        parsed = _parse_da_spedire_row(row, ri, col_map)
        if parsed is None:
            empty_run += 1
            if empty_run >= 2:
                break
