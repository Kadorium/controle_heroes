...
def _resolve_heroes_product(
    ...
    """Resolve SKU apenas via confirmação explícita no staging (sem auto-vínculo)."""
...
def _merge_preview_into_importation(
    ...
    *,
    ...
    for da in preview.get("da_spedire") or []:
        ...
        # Sconto DA SPEDIRE → HeroesDispatchPendingItem.discount_unit (via persist_dispatch_pending_items).
        # ImportationItem.discount_amount_foreign: não preencher até regra Epic (P1-c).
        existing_item = item_by_product_id.get(prod.id)
        if existing_item is None:
            new_item = ImportationItem(
                importation_id=imp.id,
...
def commit_heroes_import_run(
    ...
    *,
    ...
    run.preview_json = preview
    flag_modified(run, "preview_json")
    if da_spedire_skipped:
        preview["da_spedire_skipped"] = True
        run.preview_json = preview
        flag_modified(run, "preview_json")
