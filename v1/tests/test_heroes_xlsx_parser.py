...
def _confirm_all_staging_skus(db, run) -> None:
    """Resolve todos os grupos SKU antes do commit (triage obrigatório)."""
    from app.models import Product
    from app.services.heroes_xlsx_staging import build_sku_triage_groups, resolve_staging_sku

    groups = build_sku_triage_groups(db, run_id=run.id, raw_file_id=run.raw_file_id)
    for g in groups:
        if g.get("resolved_product_id"):
            continue
        product_id = g.get("suggested_product_id")
        if not product_id:
            raw_name = g.get("product_name_raw") or "item"
            prod = Product(
                sku_code=f"TEST-{run.id}-{g['staging_id']}"[:60],
...
def test_commit_creates_order(admin_client, db):
    ...
    _confirm_all_staging_skus(db, run)
...
def test_idempotency_same_sheet(db):
    ...
    _confirm_all_staging_skus(db, run1)
