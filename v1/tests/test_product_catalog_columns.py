...
def test_product_size_color_crud(db, admin_client):
    sku = f"CAT-SZ-{uuid.uuid4().hex[:8]}"
    r = admin_client.post(
        "/api/products",
        json={
            "sku_code": sku,
            "description": "Produto com tamanho e cor",
            "product_group": "Raquetes",
            "lifecycle_status": "ACTIVE",
            "size": "M",
            "color": "Azul",
    ...
    pid = body["id"]
    r2 = admin_client.patch(f"/api/products/{pid}", json={"size": "L", "color": "Vermelho"})
    assert r2.status_code == 200
    assert r2.json()["size"] == "L"
    assert r2.json()["color"] == "Vermelho"
