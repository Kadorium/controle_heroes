...
def test_c_staging_and_commit_block_resolve_flow(admin_client, db):
    ...
    sku_rows = [s for s in staging if (s.parsed_data_json or {}).get("issue_type") == "SKU_UNRESOLVED"]
    if not sku_rows:
    ...
    for row in sku_rows[1:]:
        row_data = row.parsed_data_json or {}
        extra = Product(
            sku_code=f"RES-{uuid.uuid4().hex[:5]}",
            description=row_data.get("product_name_raw") or "item",
            supplier_code=str(row_data.get("product_name_raw") or "").lower(),
        )
        db.add(extra)
        db.flush()
        admin_client.patch(
            f"/api/imports/staging/{row.id}/resolve-sku",
    ...
    if open_left:
        pytest.skip("Ainda há SKUs pendentes após resolve")
