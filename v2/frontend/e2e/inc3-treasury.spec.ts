...
test("Inc-3 payment allocate flow", async ({ page }) => {
  ...
  await expect(page.getByText(/Residual 1000/i)).toBeVisible();
  ...
  await expect(page.getByText(/Residual 600\.00/i)).toBeVisible({ timeout: 10000 });

  // Alocar 300 no payable que ainda tem saldo ≥300 (B €700)
  const rowB = table.locator("tbody tr").filter({ hasText: /700/ });
  await rowB.locator("input").fill("300");
  page.once("dialog", (d) => d.accept());
  await page.getByTestId("allocate-btn").click();
  await expect(page.getByText(/Residual 300\.00/i)).toBeVisible({ timeout: 10000 });
